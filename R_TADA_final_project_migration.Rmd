...
author: "Amir Firestone (192245), Ofer Dotan (195169), Jan Panhuysen (191679)"
date: "20/12/2020"
output:
...
```
<font size="2">Figure 1: Prevalence of immigration debates over time, using the number of words as a proxy for time spent on debating.</font>
...
ggplot(sum_by_party, aes(x = month, y = party, fill = party)) +
  ...
  labs(title = "Contributions to immigration-related debates by party",
       x= "Year (by month)",
       y= "Party")+
...
```
<br>
<font size="2">Figure 2: Concentration of party-specific contributions.</font>
...
```{r checking exclusivity, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

dotchart(exclusivity(topic_mod_1), labels = 1:6, color = 1:6)

```

```{r checking coherence, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r topic_by_party_plot, echo=FALSE, message=FALSE}
...
topic_by_party_plot <- party_prop_long %>%
  ...
  scale_y_continuous() + 
  scale_fill_manual(values= rev(RColorBrewer::brewer.pal(6, "Spectral")))+
...
```
<br>
<font size="2">Figure 3: Topics by party.</font>
...
k <- ggplot()  +
  ...
  scale_color_manual(name = "", values = c(part.col, "Average Prevalence" = "black"))+
  theme(axis.text.x = element_text(angle = 90))+
  facet_wrap(~topic, labeller = label_wrap_gen(width = 25, multi_line = TRUE))

k
```
<br>
<font size="2">Figure 4: Topic prevalence across time by party.</font>
...
```
<font size="2">Figure 5: Observed sentiment by party.</font>
...
```
<font size="2">Figure 6: Observed sentiment in the context of keywords.</font>
...
cor_sent_top
```
<font size="2">Table 1: Correlation of sentiments and topics.</font>
