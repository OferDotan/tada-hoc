...
# LOAD DATA #
#############
setwd("C:/Users/afirestone/Desktop/tada-hoc")

Corp_HouseOfCommons_V2  <- readRDS("Corp_HouseOfCommons_V2.rds")
names(Corp_HouseOfCommons_V2)

# for saving and loading as RData file (much smaller file and easier to load and run)
save(Corp_HouseOfCommons_V2, file = "Corp_HouseOfCommons_V2.RData")
load("Corp_HouseOfCommons_V2.RData")
...
longdf_theta_month <- longdf_theta %>%
  mutate(year_month = str_sub(date, 1, 7)) %>%
  group_by(year_month, topic) %>% 
  summarise(theta.means = mean(theta))

# plot means over months
ggplot(longdf_theta_month,aes(x=year_month, y=theta.means, group = 1)) + 
  geom_point() +
  geom_line() +
  geom_vline(xintercept = "2015-05", linetype = "dashed")+ #general election 2015
  geom_vline(xintercept = "2016-06", linetype = "dashed", color = "red")+ # Brexit referendum
  facet_grid(topic ~ .)

# plot means over years
...
names(longdf_theta_month)
#########
# mutate(year.month = str_c(str_split(date, "-")[1:2]))
#########

# plot proportions over month.years
#ggplot(longdf_theta_month,aes(x=date, y=theta.means, group=1)) +
#  geom_point() +
#  geom_line() +
#  facet_grid(topic ~ .)
