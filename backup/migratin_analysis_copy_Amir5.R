...
#agenda_text_filter$text = removeWords(agenda_text_filter$text, stopwords()) #removes stopwords from dataframe
#agenda_text_filter$text = stripWhitespace(agenda_text_filter$text) # deletes white spaces generated by stopword-deletion
...
length(docvars(speech_dfm)$party)
...
thoughts_1 <- findThoughts(mod_1, texts = texts(speechcorp), topics = 1)



#merge text to to df_party_mean or similar df



head(docvars(speech_dfm)$terms)

plotQuote(thoughts_1, width = 30, main = "Topic 1")

length(texts(speechcorp))
length(mod_1$theta[,1])
...
## code from online that doesnt really work here...
rowTotals <- apply(speechcorp, 1, sum) #Find the sum of words in each Document
speechcorp.new   <- speechcorp[rowTotals> 0, ]           #remove all docs without words
