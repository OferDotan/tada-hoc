...
ggplot(doc_sum_by_words, aes(x=month, y=word_sum)) +
  geom_area( fill="#69b3a2", alpha=0.4) +
  geom_line(color="#69b3a2") +
  labs(title = "Prevalence of immigration debates",
...
### to make those graphs more interactive jl

# create xts necessary to use dygraph (from here it would replace the ggplot part)
xts_dygraph <- xts(x = doc_sum_by_words$word_sum, order.by = doc_sum_by_words$month)
names(xts_dygraph) <- "# words"

# Plot
interactive_graph <- dygraph(xts_dygraph, 
             main = "Prevalence of immigration debates (as 6-month average)", 
...
# group by party and generate means 
theta_data_party <- HoC_theta_data %>%
  group_by(party)
...
# turn into a table of proportions
theta_data_party_prop <- theta_data_party_mean[1] %>%
  cbind(as.data.frame(
    round(100*prop.table(theta_data_party_mean[2:7]),
          digits=2)))

# pivot df for making figure
party_prop_long <- theta_data_party_prop %>%
  pivot_longer(c(2:7), 
               names_to = "Topic", 
               values_to = "Proportion")

# visualize proportions of topics by party
topic_by_party_plot <- party_prop_long %>%
  ggplot(aes(Party, Proportion, fill = Topic)) +
  geom_bar(position = "fill", stat = "identity") +
  scale_y_continuous() + 
...
ggplot(theta_long_data_year_1,aes(x=year, y=theta.means, group = 1, color = topic)) + 
  geom_point() +
  geom_line() +
  geom_vline(xintercept = "2015", linetype = "dashed")+ #general election 2015
  geom_vline(xintercept = "2016", linetype = "dashed", color = "red")+ # Brexit referendum
