...
speech_dfm <- dfm_trim(speech_dfm,
                     min_termfreq = 2, min_docfreq = 2)

speech_stm <-asSTMCorpus(speech_dfm)
```
...
mod_3 <- stm(speech_stm$documents, vocab = speech_stm$vocab, 
             K = 6, ~ party, data=speech_stm$data, seed = 12345)
...
mod_3 <- estimateEffect(1:6~party, mod_1)

#plot to see exclusivity of topics
plot(topicCorr(mod_3))
#plot to chec
topicQuality(mod_3, documents = speech_stm$documents)
dim(mod3$beta$logbeta[[1]])

#code to add labels
labels <- apply(sageLabels(mod_1)$marginal$frex, 1,
