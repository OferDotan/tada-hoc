...
sentiment_long_party <- sentiment_long_party %>%
  filter(party == "Con" | party == "Lab" | party == "GPEW" | party == "Independent" | party == "LibDem" | party == "PlaidCymru" | party == "SDLP" | party == "SNP")
...
og_reg <- lm(sentiment.means~theta.means+year, data=sentiment_long_party)
summary(og_reg)
...
# make a nice stargazer table of these (but first decide which controls to use or to do nested models)
```
...
# this is supposed to work for plotting multivariate regression but it doesn't work
ggPredict(fit,se=TRUE,interactive=FALSE)

# this is also supposed to work but I didn't get it to... not sure about the function, the line is contant for all topics :(
equation1=function(x){coef(reg_0)[2]*x+coef(reg_0)[1]+coef(reg_0)[3]}

ggplot(sentiment_long_party,aes(y=sentiment.means,x=theta.means,color=topic))+geom_point()+
        stat_function(fun=equation1,geom="line",color=scales::hue_pal()(2)[1])+
  facet_wrap(~topic)
      

# for all parties (color)
ggplot(sentiment_long_party,aes(y=sentiment.means,x=theta.means, color = party, fill = party)) +
  geom_point() +
...
# for all parties (no color) but want to control for party and year! (seems geom_smooth does not allow multivariate regression formulas)
ggplot(sentiment_long_party,aes(y=sentiment.means,x=theta.means)) +
  geom_point() +
  geom_smooth(method="lm", show.legend = FALSE) +
  theme_bw() + 
  xlab("Topic Theta Mean") +
...
# want to control for party and year!

# for just topic 4, comparing individual parties (no color)
ggplot(sentiment_long_party_topic4,aes(y=sentiment.means,x=theta.means)) +
  geom_point() +
  geom_smooth(method="lm", show.legend = FALSE) +
  theme_bw() + 
  xlab("Topic Theta Mean") +
