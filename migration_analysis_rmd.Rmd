...
og_reg <- lm(sentiment~theta+date+party, data=theta_data_long_reg)
summary(og_reg)
...
ggplot(theta_data_long_reg,aes(y=sentiment,x=theta,color=topic))+geom_point()+
...
ggplot(sentiment_topic4,aes(y=sentiment,x=theta)) +
  geom_point() +
  geom_smooth(method="lm", show.legend = FALSE) +
  theme_bw() + 
  xlab("Topic Theta") +
