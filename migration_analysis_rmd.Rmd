...
HoC_corp <- corpus(HoC_data) # re-run here to include sentiment
...
thoughts <- findThoughts(topic_mod_1, texts = texts(HoC_corp), n=2, topics = 1:6)#n=how many documents --> increasing n doesn't seem to have an effect.
#plot thoughts for topics
stm::plotQuote(thoughts$docs$`Topic 4`, width = 80, text.cex=0.6)
save(thoughts, file = "thoughts.RData")

#kwic for thoughts
thoughts_1 <- data.frame(t(as.data.frame(thoughts$docs)))
names(thoughts_1) <- "text"

thoughts <- corpus(thoughts_1)
thoughts_kwic <- kwic(thoughts, 
                 paste(key_words_img,collapse="|"), 
                 window = 20)

thoughts_kwic_df <- tibble(speaker = thoughts_kwic$docname, 
                  text = paste(thoughts_kwic$pre,
                               thoughts_kwic$keyword,
                               thoughts_kwic$post, sep = " "))
